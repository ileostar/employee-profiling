    ...
    <div class="account-manage">
        ...
        <div class="account-table my-table">     
            <el-table :data="tableData" stripe style="width: 100%">
                <el-table-column v-for="prop,index of tableProps" :key="index" :prop="prop" :label="prop" :width="prop === 'createdTime' ? 230 : 100" />
                <el-table-column align="right">
                  <template #default="scope">
                      <el-button link type="info" size="small" @click="handleClick(scope.row)" >修改</el-button>       
                      <el-popconfirm title="你确定要删除当前用户吗?" width="220" @confirm="deleteIndexUser(scope.row)">
                        <template #reference>
        ...
        <el-dialog v-model="dialogFormVisible" title="修改用户信息">
          <el-form :model="ruleForm" ref="ruleFormRef" :rules="rules">
            <el-form-item label="用户名" :label-width="formLabelWidth">
              <el-input v-model="ruleForm.username" prop="username" :placeholder="currenRowData.username" autocomplete="off" />
            </el-form-item>
...
const tableProps: string[] = Object.keys(toRaw(tableData.value[0]))?.filter((item: unknown)=> item!=='password'&&item!=='salt')
